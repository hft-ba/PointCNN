#!/usr/bin/python3
"""Training, Validation and Test On Classification Task."""
...
def main():
    ...
    parser.add_argument('--path_test', '-j', help='Path to test data')
    ...
    with tf.Session() as sess:
        ...
        # load test data
        data_test, label_test = data_utils.load_cls(args.path_test)

        num_test = data_test.shape[0]
        print('{}-num_test               : {}'.format(datetime.now(), num_test))
        ...
        if setting.map_fn is not None:
            dataset_test = dataset_test.map(lambda data, label: tuple(tf.py_func(
                setting.map_fn, [data, label], [tf.float32, label.dtype])), num_parallel_calls=setting.num_parallel_calls)
        if setting.keep_remainder:
            dataset_test = dataset_test.batch(batch_size)
            batch_num_test = math.ceil(num_test / batch_size)
        else:
        ...
        for batch_idx_test in range(batch_num_test):
            if not setting.keep_remainder \
                    or num_test % batch_size == 0 \
                    or batch_idx_test != batch_num_test - 1:
            ...
            sess.run([loss_mean_update_op, t_1_acc_update_op, t_1_per_class_acc_update_op],
                     feed_dict={
                         handle: handle_test,
                         indices: pf.get_indices(batch_size_test, sample_num, point_num,
                                                 ),
                         xforms: xforms_np,
                         rotations: rotations_np,
                         jitter_range: np.array([setting.jitter_test]),
                         is_training: False,
                     })
        loss_test, t_1_acc_test, t_1_per_class_acc_test, summaries_test, step = sess.run(
            [loss_mean_op, t_1_acc_op, t_1_per_class_acc_op, summaries_val_op, global_step])


        summary_writer.add_summary(summaries_test, step)
        print('{}-[Test ]-Average:      Loss: {:.4f}  T-1 Acc: {:.4f}  T-1 mAcc: {:.4f}'
              .format(datetime.now(), loss_test, t_1_acc_test, t_1_per_class_acc_test))
        sys.stdout.flush()
